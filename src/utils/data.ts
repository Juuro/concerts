...
import type {
  ContentfulConcertEntry,
  ContentfulBandEntry,
  ContentfulCity,
  ContentfulFestivalEntry,
  ContentfulConcertFields,
  ContentfulBandFields,
...
let lastfmCachePromise: Promise<Map<string, LastFMArtistInfoOrNull>> | null =
  null
...
async function transformConcert(
  entry: ContentfulConcertEntry
): Promise<Concert> {
  const fields = getConcertFields(entry)
  const city = getCity(fields)
  const geocodingData = await getGeocodingData(city.lat, city.lon)

  const lastfmByArtist = await loadLastfmCache()

  const bands: ContentfulBandEntry[] =
    (fields.bands as ContentfulBandEntry[] | undefined) || []
  const bandsFormatted = await Promise.all(
    bands.map(async (band: ContentfulBandEntry) => {
      const bandFields = getBandFields(band)
      const lastfm =
        lastfmByArtist.get(normalizeLastfmKey(bandFields.name)) ?? null
      return {
        id: band.sys.id,
    ...
    try {
      ...
      const concerts = await Promise.all(
        entries.items.map((entry) =>
          transformConcert(entry as ContentfulConcertEntry)
        )
      )
