...
type LeafletContainer = HTMLDivElement & {
  _leaflet_id?: number | string | null
}
type MapboxTileLayerOptions = TileLayerOptions & {
  id: string
  accessToken: string
}
...
export default function MapClient({ concerts }: MapClientProps) {
  ...
  useEffect(() => {
    ...
    import("leaflet").then((leafletModule) => {
      const Leaflet =
        (leafletModule as LeafletModule & { default?: LeafletModule })
      ...
      import("leaflet/dist/images/marker-icon.png").then((markerIconModule) => {
        import("leaflet.markercluster").then(() => {
          ...
          const icon = new Leaflet.Icon({
            iconUrl: (markerIconModule as { default: StaticImageData }).default
              .src,
            iconSize: [25, 41],
            iconAnchor: [12, 41],
            popupAnchor: [1, -34],
          ...
          concerts.forEach((concert) => {
            const marker = Leaflet.marker(
              [concert.city.lat, concert.city.lon],
              {
                icon: icon,
              }
            )
